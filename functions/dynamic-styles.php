...
/* ------------------------------------ */
if ( ! function_exists( 'personalias_hex2rgb' ) ) {

	function personalias_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'personalias_enqueue_google_fonts' ) ) {

	function personalias_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'personalias_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'personalias_dynamic_css' ) ) {

	function personalias_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = personalias_hex2rgb($color_1);
...
#wrapper { border-color: '.esc_attr( get_theme_mod('color-header') ).'; }
@media only screen and (max-width: 719px) {
	.site-title,
	#header .social-links { border-bottom: 1px solid rgba(255,255,255,0.2)!important; }
}
				'."\n";
			}
			// footer color
			if ( get_theme_mod('color-footer','#000000') != '#000000' ) {
				$styles .= '
#footer-bottom { background: '.esc_attr( get_theme_mod('color-footer') ).'; }
				'."\n";
			}
			...
			wp_add_inline_style( 'personalias-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'personalias_dynamic_css' );
