<div class="page-title group">
	<div class="page-title-inner group">
		...
		<?php if ( is_home() ) : ?>
			<h2><?php echo personalias_blog_title(); ?></h2>
		...
		<?php elseif ( is_search() ): ?>
			<h1>
				...
				<?php $search_results=$wp_query->found_posts;
					if ($search_results==1) {
						echo $search_results.' '.esc_html__('Search result','personalias');
					} else {
						echo $search_results.' '.esc_html__('Search results','personalias');
					}
				?>
			</h1>
			<div class="notebox">
				<?php esc_html_e('For the term','personalias'); ?> "<span><?php echo get_search_query(); ?></span>".
				<?php if ( !have_posts() ): ?>
					<?php esc_html_e('Please try another search:','personalias'); ?>
				<?php endif; ?>
		...
		<?php elseif ( is_404() ): ?>
			<h1><i class="fas fa-exclamation-circle"></i><?php esc_html_e('Error 404.','personalias'); ?> <span><?php esc_html_e('Page not found!','personalias'); ?></span></h1>
			<div class="notebox">	
				<p><?php esc_html_e( 'The page you are trying to reach does not exist, or has been moved. Please use the menus or the search box to find what you are looking for.', 'personalias' ); ?></p>
				<?php get_search_form(); ?>
			</div>
